...
class Tracker:
    ...
    def detect_frames(self, frames):
        ...
        for i in range(0, len(frames), batch_size):
            ...
            detections_batch = self.model.predict(frames[i:i+batch_size], conf=0.35, imgsz=1080)
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # Adicionámos a verificação "is not None" para o Pylance saber que é seguro
            ...
            for frame_detection in detection_supervision:
                ...
                if cls_id == cls_names_inv.get("ball", 32): 
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        cv2.ellipse(frame, center=(x_center, y2), axes=(int(width), int(0.35 * width)),
                    angle=0, startAngle=-45, endAngle=235, color=color, thickness=2, lineType=cv2.LINE_4)
    ...
    def draw_triangle(self, frame, bbox, color):
        y = int(bbox[1])
        x, _ = int((bbox[0] + bbox[2]) / 2), int((bbox[1] + bbox[3]) / 2)
